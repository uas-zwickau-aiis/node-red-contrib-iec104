...
const registerNode = require('../../../iec104-singlepoint');

describe('iec104-singlepoint node', function () {
  let NodeConstructor;
  let RED;
    ...
    RED = {
      nodes: {
        ...
        registerType(name, ctor) {
          assert.strictEqual(name, 'iec104-singlepoint');
  ...
  describe('successful processing', function () {
    it('creates and sends a true single point', function () {
      const msg = {
        payload: true,
        ioa: [0, 0, 1]
      };
      ...
      assert.deepStrictEqual(node.status.firstCall.args[0], {});
    });

    it('creates and sends a false single point', function () {
      const msg = {
        payload: false,
        ioa: [0, 0, 1]
      };
    ...
    it('uses M_SP_NA_1 as default object type', function () {
      const msg = {
        payload: true,
        ioa: [0, 0, 1]
      };
    ...
    it('accepts string "true"', function () {
      const msg = {
        payload: 'true',
        ioa: [0, 0, 1]
      };
    ...
    it('accepts string "false"', function () {
      const msg = {
        payload: 'false',
        ioa: [0, 0, 1]
      };
    ...
    it('normalizes whitespace and casing for boolean strings', function () {
      const msg = {
        payload: '  TRUE  ',
        ioa: [0, 0, 1]
      };
  ...
  describe('value validation', function () {
    ...
    it('rejects invalid string value', function () {
      const msg = {
        payload: 'yes',
        ioa: [0, 0, 1]
      };
  ...
  describe('timestamp handling', function () {
    it('does not add timestamp for M_SP_NA_1', function () {
      const msg = {
        payload: true,
        ioa: [0, 0, 1]
      };
    ...
    it('adds current timestamp for timed single point type', function () {
      ...
      assert.strictEqual(Number.isFinite(parsed), true);
