...
class APCI {
    ...
    isAckValid(nr) {
        const sentDistance = (this.sendSeq - this.ackSeq + SEQ_MOD) % SEQ_MOD;
        const ackDistance  = (nr - this.ackSeq + SEQ_MOD) % SEQ_MOD;

        return ackDistance <= sentDistance;
    }
   ...
   updateRecvFromFrame(buf) {
        ...
        if (!this.isAckValid(nr)) {
            throw new Error(`Invalid ACK sequence: ${nr}`);
        }
        ...
        if (this.isIFrame(buf)) {
            ...
            if (ns !== this.recvSeq) {
                throw new Error(`Unexpected N(S): got ${ns}, expected ${this.recvSeq}`);
            }

            this.recvSeq = this.incSeq(this.recvSeq);
    ...
    getStatus() {
        return {
            ...
            recvSinceLastAck: this.recvSinceLastAck,
            k: this.k,
            w: this.w,
            hasSendWindow: this.hasSendWindow()
        };
    }
