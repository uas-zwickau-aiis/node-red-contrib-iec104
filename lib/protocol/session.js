...
class Session {
    constructor(opts = {}) {
        ...
        this.giInProgress = false; // Schutz vor doppeltem GI
    ...
    async handleFrame(buf) {
        ...
        if (typeId === IEC104.ASDU.C_IC_NA_1 && cot === IEC104.COT.ACT) {
              ...
              if (this.giInProgress) {
                // Zweite GI → nur bestätigen & terminieren
    ...
    sendPoint(p, cause = "SPONT") {
        ...
        const asdu = buildASDU(p, cause, this.ca);
        if (!asdu) return;
