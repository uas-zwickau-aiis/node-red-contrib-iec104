...
class SlaveSession extends BaseSession {
    ...
    async handleASDU(asdu) {
        const { typeId, cot } = asdu;

        if (
            typeId === TYPES.C_IC_NA_1.id &&
            cot === COT.ACT
        ) {
            await this.handleInterrogation(asdu);
            return;
        }
        ...
        if (
            ...
            await this.handleSingleCommand(asdu);
        }
    }

    async handleInterrogation(asdu) {
        const { ca } = asdu;
        ...
        try {
            ...
            await this.onGI(ca, async point => {
                while (!this.apci.hasSendWindow()) {
    ...
    async handleSingleCommand(asdu) {
        await this.onCommand(asdu);
    }

    sendInterrogationResponse(cause, ca) {
        const frame =
            this.apci.buildInterrogationFrame(
                cause,
                ca
            );

        this.sendFrame(frame, FRAME.I);
